	...
	if (tag == "light") {
		...
		for (var i = 0; i < pro.length; i++) {
			...
			if (tmpObj.checked) {
				...
				str1 += tmpObj.value;
				str2 += tmpObj.id;
				html_select += "<option value='" + tmpObj.id + "'>"
						+ tmpObj.value + "</option>";
			} else {
...
function mainOpen() {
	...
	var url = "/NfCrm/pubcustomer/redirectProPan.do";
	window
			.open(
					url,
					"child",
...
function change() {
	var customer_budget_min = $(":input[name='customer_budget_min']").val();
	var customer_budget_max = $(":input[name='customer_budget_max']").val();
	if (customer_budget_min >= 0 && customer_budget_max <= 50) {
		$("#CustomerLevel").find("option[value='C']").attr("selected", true);
	} else if (customer_budget_min > 50 && customer_budget_max <= 100) {
		$("#CustomerLevel").find("option[value='B']").attr("selected", true);
	} else {
		$("#CustomerLevel").find("option[value='A']").attr("selected", true);
	}
}
/**
 * 验证客户是否存在
...
function checkperson() {
	var recommend_person = encodeURI($(":input[name='recommend_person']").val());
	$.ajax({
		type : "Post",
		url : "/NfCrm/pubcustomer/queryCustomerName.do",
		data : "recommend_person=" + recommend_person,
		success : function(msg) {
			if (msg == 1) {
				$("#person").html("推荐人不存在").css("color", "red");
				$("#recommend_person").val("");
				return false;
			} else {
				$("#person").html("").css("color", "red");
				return true;
			}
		},
		error : function(rec) {
...
$(document)
		.ready(
				function() {
					...
					// 拼接客户预算
					/*
					 * $("#level1,#level2").blur(function(){ var
					 * State=$("#level1").val(); var End=$("#level2").val();
					 * if(State!=""&&End!=""){ $("#armText").val(State+"-"+End); }
					 * });
					 */
					...
					$("#up_button").click(function() {
						...
						$.ajax({
							type : "POST",
							url : "/NfCrm/pubcustomer/upload.do",
							data : formData,
							processData : false,
							contentType : false,
							datetype : "json",
							...
							.click(
									function() {
										var pro_id = $("input[name='pro_id']")
												.val();
										var follow_time = $(
												"input[name='follow_time']")
												.val();
										var follow_desc = $(
												"input[name='follow_desc']")
												.val();
										var repeat_follow = $(
												"input[name='repeat_follow']")
												.val();
										var follow_scheme = $(
												"input[name='follow_scheme']")
												.val();

										if (pro_id == "" || follow_time == ""
												|| follow_desc == ""
												|| repeat_follow == ""
												|| follow_scheme == "") {
											swal({
											...
											var data = {
												...
												follow_img : $("#pho").attr(
														"src"),
												follow_desc : $(
														"textarea[name='follow_desc']")
														.val(),
												repeat_follow : $(
														"input[name='repeat_follow']")
														.val(),
												follow_scheme : $(
														"textarea[name='follow_scheme']")
														.val(),
											};
											$
													.ajax({
																	...
																	.ajax({
																		...
																		success : function(
																			...
																			for (var i = 0; i < datamap.length; i++) {
																				...
																				var son = "<tr><td>"
																						+ datamap[i].follow_time
																						+ "</td><td>"
																						+ datamap[i].follow_pro
																						+ "</td><td>"
																						+ record
																						+ "</td><td>"
																						+ datamap[i].follow_desc
																						+ "</td><td>"
																						+ img
																						+ "</td><td>"
																						+ datamap[i].repeat_follow
																						+ "</td><td>"
																						+ "<input type='button' id="
																						+ logId
																						+ " name='logDetail' value='查看' onclick='LogDetail("
																						+ logId
																						+ ")'>"
																						+ "</td></tr>";
																				$(
					...
					$.ajax({
						type : "POST",
						url : "/NfCrm/customer/queryloginfo.do",
						data : data3,
						async : true,
					...
					$("#zcq").click(
							function() {
								var recommend_name = $(
										"input[name='recommend_name']").val();
								if (recommend_name != "") {
									var newE = "<tr><td name='Rn'>"
											+ recommend_name + "</td></tr>";
							...
							.click(
									function() {
										...
										$
												.ajax({
													type : "POST",
													url : "/NfCrm/customer/selectCustomerName.do",
													data : data,
													async : true,
													dataType : "json",
													success : function(data) {
